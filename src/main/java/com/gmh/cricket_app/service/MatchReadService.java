...
public class MatchReadService {
    ...
    public List<MatchSummary> getMatchList(String sessionToken) {
        ...
        List<Match> matches = matchRepo.findByHostedByUserId(user.getId());
        return toSummaries(matches);
    }

    public List<MatchSummary> getRecentMatches(String sessionToken) {
        User user = sessionService.validateSession(sessionToken);
        List<Match> matches = matchRepo.findByHostedByUserIdOrderByPlannedStartTimeDesc(user.getId());
        return toSummaries(matches);
    }

    private List<MatchSummary> toSummaries(List<Match> matches) {
        Set<String> teamIds = matches.stream()
                .flatMap(m -> java.util.stream.Stream.of(m.getTeamAId(), m.getTeamBId()))
                .collect(Collectors.toSet());
        Map<String, String> teamNames = teamRepo.findAllById(teamIds).stream()
                .collect(Collectors.toMap(Team::getId, Team::getName));
        return matches.stream()
                .map(m -> new MatchSummary(
                        m.getId(), m.getFormat(), m.getStatus(), m.getTotalOvers(),
                        m.getTeamAId(), teamNames.get(m.getTeamAId()),
                        m.getTeamBId(), teamNames.get(m.getTeamBId()),
    ...
    public LiveStateResponse getLiveState(String sessionToken, String matchId) {
        ...
        ActiveInningsSummary inningsSummary = new ActiveInningsSummary(
                ...
                teamNames.get(activeInnings.getBattingTeamId()),
                activeInnings.getBowlingTeamId(),
                teamNames.get(activeInnings.getBowlingTeamId()),
        ...
        ActiveOverSummary overSummary = activeOver == null ? null : new ActiveOverSummary(
                ...
                playerNames.get(activeOver.getBowlerId()),
        ...
        String lastBatsmanId = lastBall != null ? lastBall.getBatsmanId() : null;
        String lastNonStrikerId = lastBall != null ? lastBall.getNonStrikerId() : null;
        ...
        return new LiveStateResponse(
                ...
                lastBatsmanId, playerNames.get(lastBatsmanId),
                lastNonStrikerId, playerNames.get(lastNonStrikerId)
        );
    }
}
